# this code takes the microphone output and saves it to excel
# the pico should be running the code main_mic.py
...
def read_from_serial():
    timestamp = 0.0  # Initialize timestamp
    while running:
        try:
            ...
            if line:
                try:
                    value = float(line)
                    print(f"Time: {timestamp:.2f} s, Received line: {value:.2f}")
                    data.append({'Time (s)': timestamp, 'Received line': value})
                    timestamp += 0.05  # Increment timestamp by 0.05 seconds
                except ValueError:
